...
class ConvLayer:
    def __init__(self, name, m_in, m_out, apply_batch_norm, filter_sz=5, strides=2,f=tf.nn.relu):
        ...
        :param m_in: input size
        :param m_out:  output size
        :param apply_batch_norm: (true or false) used batch norm or not
        :param filter_sz: the size of kernel
        :param strides:
        ...
        self.b = tf.get_variable("b_%s"%name, shape=(m_out,), initializer=tf.zeros_initializer)
...
class FractionallyStrideConvLayer:
    def __init__(self, name, m_in, m_out, output_shape, apply_batch_norm, filtersz=5, stride=2, f=tf.nn.relu):
        ...
        # NOTE !!! sahpe is specified in the OPPOSITE way from regular conv
        # self.W = tf.Variable(0.02 * tf.random_normal(shape=(filtersz, filtersz, m_out, m_in)))
    ...
    def build_generator(self, Z, g_sizes):
        with tf.variable_scope("generator") as scope:
            ...
            for i in range(len(g_sizes['conv_layers'])):
                ...
                layer = FractionallyStrideConvLayer(
...
def mnist():
    ...
    # setup gan
    # note: assume squar images, so only need 1 dim
    gan = DCGAN(dim, colors, d_sizes, g_sizes)
    gan.fit(X)
